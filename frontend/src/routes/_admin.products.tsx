...
function ProductsPage() {
  ...
  function exportPdf() {
    ...
    doc.save(
      `products-report-${Date.now()}.pdf`
    );

    toast.success(
      "Products exported to PDF"
    );
  }
            ...
            <div>
              ...
              {cats.length === 0 ? (
                <p className="text-xs text-muted-foreground mt-1">
                  No{" "}
                  {p.metal}{" "}
                  categories yet — add one on the Categories page first.
                </p>
              ) : (
                <Select
                  value={
